#!/usr/bin/env python3
"""
Eye-in-Hand Calibration Unit Tests
==================================

This module contains unit tests for the EyeInHandCalibrator class.
Tests are adapted from the examples and provide comprehensive validation
of the eye-in-hand calibration workflow.

Test Coverage:
- Intrinsic calibration prerequisite
- Data loading and validation
- Eye-in-hand calibration execution
...
class TestEyeInHandCalibration(unittest.TestCase):
    """
    Unit test class for EyeInHandCalibrator functionality.
    
    This test class validates the EyeInHandCalibrator's ability to perform
    complete eye-in-hand calibration workflows including data loading,
    ...
    def setUpClass(cls):
        ...
        cls.data_dir = os.path.join("sample_data", "eye_in_hand_test_data")
        cls.results_dir = tempfile.mkdtemp(prefix="eye_in_hand_test_")
    ...
    def test_eye_in_hand_calibration(self):
        """
        ...
        # Validate intrinsic calibration quality
        self.assertLess(intrinsic_rms, 0.5, f"Intrinsic calibration RMS error too high: {intrinsic_rms:.4f} > 0.5")
        
        print(f"✅ Intrinsic calibration passed quality check (RMS: {intrinsic_rms:.4f})")
        
        # Step 2: Load eye-in-hand data
        images, end2base_matrices = self.load_data(self.data_dir)
        ...
        pattern_config_path = os.path.join(self.data_dir, "chessboard_config.json")
        with open(pattern_config_path, 'r') as f:
            pattern_json_data = json.load(f)
        calibration_pattern = load_pattern_from_json(pattern_json_data)
        
        # Step 4: Initialize eye-in-hand calibrator
        ...
        self.assertIsNotNone(calibrator, "Failed to create EyeInHandCalibrator")
        
        # Step 5: Perform calibration
        print(f"🔍 Starting eye-in-hand calibration with {len(images)} image-pose pairs...")
        
        result = calibrator.calibrate(verbose=True)
        
        self.assertIsNotNone(result, "Eye-in-hand calibration returned None")
        self.assertIsInstance(result, dict, "Calibration result should be a dictionary")
        
        # Step 6: Validate calibration results
        required_keys = ['cam2end_matrix', 'target2base_matrix', 'rms_error']
        ...
        cam2end_matrix = result['cam2end_matrix']
        target2base_matrix = result['target2base_matrix']
        
        # Validate error threshold
        self.assertLess(rms_error, 1.0, f"Eye-in-hand calibration RMS error too high: {rms_error:.4f} > 1.0")
        
        # Validate matrix formats
        self.assertEqual(cam2end_matrix.shape, (4, 4), "cam2end_matrix should be 4x4")
        self.assertEqual(target2base_matrix.shape, (4, 4), "target2base_matrix should be 4x4")
        
        # Validate transformation matrix properties
        np.testing.assert_allclose(cam2end_matrix[3, :], [0, 0, 0, 1], atol=1e-6,
                                 err_msg="cam2end_matrix bottom row should be [0, 0, 0, 1]")
        np.testing.assert_allclose(target2base_matrix[3, :], [0, 0, 0, 1], atol=1e-6,
                                 err_msg="target2base_matrix bottom row should be [0, 0, 0, 1]")
        
        print(f"✅ Eye-in-hand calibration successful!")
        print(f"📊 RMS Error: {rms_error:.4f} pixels")
        print(f"🔗 Camera to End-Effector transformation matrix:")
        print(f"{cam2end_matrix}")
        print(f"🎯 Target to Base transformation matrix:")
        print(f"{target2base_matrix}")
        
        # Step 7: Generate calibration report
        hand_eye_results_dir = os.path.join(self.results_dir, "eye_in_hand_calibration")
        ...
        print(f"📋 Eye-in-hand calibration report generated: {report_info}")
        ...
        new_calibrator = EyeInHandCalibrator()
        new_calibrator.from_json(calibrator_json)
        
        # Verify loaded matrices match
        np.testing.assert_array_almost_equal(
            new_calibrator.get_cam2end_matrix(), cam2end_matrix,
            err_msg="Loaded cam2end_matrix doesn't match original"
        )
        np.testing.assert_array_almost_equal(
            new_calibrator.get_target2base_matrix(), target2base_matrix,
            err_msg="Loaded target2base_matrix doesn't match original"
        )
        
        print(f"✅ JSON serialization/deserialization test passed")
        
        print(f"🎉 All eye-in-hand calibration tests passed!")
